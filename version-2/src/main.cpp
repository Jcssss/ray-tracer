...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce, int object_source);
...
double dotGridGradient(int ix, int iy, double x, double y)
{
    ...
    const Vector2d dist_vec(ix - x, iy - y);

    //TODO: Compute and return the dot-product
    return dist_vec.dot(grid[ix][iy]);
}
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position, const int &object_num)
{
    ...
    const double dist_to_light = (light_position - ray_origin).norm();
    const double dist_to_object = (p - ray_origin).norm();

    return nearest_object == -1;// || dist_to_object > dist_to_light ;
}
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce, int object_source)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        const Vector4d specular = obj_specular_color * pow(std::max(0., n.dot(h)), obj_specular_exponent);
    ...
    if (max_bounce > 0 && object_source == -1) {
        const Vector3d d = (p - ray_origin).normalized();
        const Vector3d n = N;

        const Vector3d r = (d - 2 * d.dot(n) * n).normalized();
        const Vector4d ray_color = shoot_ray(p + 0.001 * r, r, max_bounce - 1, -1);
    ...
    double refraction_index_1 = 0;
    double refraction_index_2 = 0;
    int next_source = -1;

    if (nearest_object != -1 && object_source == -1) {

        refraction_index_1 = 1;
        refraction_index_2 = 1.52;
        next_source = nearest_object;

    } else if (nearest_object != -1 && nearest_object == object_source) {

        refraction_index_1 = 1.52;
        refraction_index_2 = 1;

    } else if (nearest_object != -1 && object_source != -1 && nearest_object != object_source) {

        refraction_index_1 = 1.52;
        refraction_index_2 = 1.52;
        next_source = object_source;

    }

    if (refraction_index_1 != 0) {
        const Vector3d v = (camera_position - p).normalized();
        Vector3d n = N;

        double theta_1 = acos(v.dot(n));
        if (object_source != -1) {
            theta_1 = acos(-v.dot(n));
            n = -N;
        }

        const double theta_2 = asin(sin(theta_1) * refraction_index_1 / refraction_index_2);

        Vector3d t = refraction_index_1 / refraction_index_2 * (-v + cos(theta_1) * n) - n * cos(theta_2);
        t = t.normalized();
        
        const Vector4d ray_color = shoot_ray(p + 0.001 * t, t, max_bounce, next_source);
 
        refraction_color[0] = ray_color[0] * obj_refraction_color[0];
        refraction_color[1] = ray_color[1] * obj_refraction_color[1];
        refraction_color[2] = ray_color[2] * obj_refraction_color[2];
        refraction_color[3] = ray_color[3] * obj_refraction_color[3];
    }
...
double generate_random() {
    return ((double) rand() / (RAND_MAX)) / 2 + (1.0 / 2);
}
...
void raytrace_scene()
{
    ...
    for (unsigned i = 0; i < w; ++i)
    {
        for (unsigned j = 0; j < h; ++j)
        {
            ...
            const std::vector<Vector3d> cameras = {
                camera_position,
                camera_position + camera_aperture * ((double) rand() / (RAND_MAX)) * x + camera_aperture * ((double) rand() / (RAND_MAX)) * y, 
                camera_position + camera_aperture * ((double) rand() / (RAND_MAX)) * x - camera_aperture * ((double) rand() / (RAND_MAX)) * y, 
                camera_position - camera_aperture * ((double) rand() / (RAND_MAX)) * x + camera_aperture * ((double) rand() / (RAND_MAX)) * y, 
                camera_position - camera_aperture * ((double) rand() / (RAND_MAX)) * x - camera_aperture * ((double) rand() / (RAND_MAX)) * y, 
            };
            ...
            Vector4d C(0,0,0,0);
            for (unsigned k = 0; k < cameras.size(); ++k)
            {
                // Prepare the ray
                Vector3d ray_origin;
                ...
                if (is_perspective)
                {
                    ...
                    ray_origin = cameras[k];
                    ray_direction = (pixel_center - cameras[k]).normalized();
                }
                else
                {
                    // Orthographic camera
                    ray_origin = cameras[k] + Vector3d(pixel_center[0], pixel_center[1], 0);
                    ray_direction = Vector3d(0, 0, -1);
                }

                C = ((k) * C + shoot_ray(ray_origin, ray_direction, max_bounce, -1)) / (k + 1);
            }
